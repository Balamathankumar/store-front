...
const HomePage: React.FC = () => {
  ...
  const { results: searchResults, loading: searchLoading, search, clearResults } = useProductSearch();
  ...
  const handleSearch = (query: string) => {
    if (query.trim()) {
      ...
      const category = activeCategory !== 'all' ? activeCategory as 'NUT' | 'DRY FRUIT' | 'SPICE' | 'SEEDS' : undefined;
  ...
  const handleCategoryChange = (category: string) => {
    ...
    if (isSearchMode && searchQuery.trim()) {
      const searchCategory = category !== 'all' ? category as 'NUT' | 'DRY FRUIT' | 'SPICE' | 'SEEDS' : undefined;
  ...
  const displayProducts = useMemo(() => {
    let products = isSearchMode && searchResults ? searchResults.items : allProducts;
    ...
    switch (activeSort) {
      ...
      case 'new':
        return sortedProducts.filter(p => p.badge === 'NEW').concat(
          sortedProducts.filter(p => p.badge !== 'NEW')
        );
      case 'combos':
        ...
        sortedProducts.forEach(p => {
          console.log(`Product ${p.id} (${p.name}): isCombo=${p.isCombo}, badge="${p.badge}"`);
        });
        
        const comboProducts = sortedProducts.filter(product => 
          (product.isCombo === true) || (product.badge === 'COMBO')
        );
        console.log('Combo filter activated. Total products:', sortedProducts.length);
        console.log('Combo products found:', comboProducts.length);
        console.log('Combo products:', comboProducts.map(p => ({ id: p.id, name: p.name, isCombo: p.isCombo, badge: p.badge })));
  ...
  const handleProductClick = (productId: number) => {
    ...
    if (!product) {
      ...
      const comboVersion = allProducts.find(p => p.id === productId && (p.isCombo === true || p.badge === 'COMBO'));
    ...
    if ((product?.isCombo === true) || (product?.badge === 'COMBO')) {
      ...
      console.log('Combo check: isCombo =', product?.isCombo, ', badge =', product?.badge);
