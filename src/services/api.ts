...
export class ApiService {
  ...
  static async getProductImages(id: number): Promise<string[]> {
    try {
      const response = await api.get(`/items/${id}/images`);
      // Expect the API to return an array of image URLs
      return response.data.images || response.data;
    } catch (error) {
      console.warn(`Failed to fetch images for product ${id}, using fallback`);
      // Return empty array to trigger fallback to emojis
      return [];
    }
  }
