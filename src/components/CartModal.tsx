...
import { useGetCustomers } from '../hooks/useApi';
...
const CartModal: React.FC = () => {
  ...
  //   if (items.length === 0) {
  //     setConfirmMessage('Your cart is empty!');
  //     return;
  //   }

  //   const accessToken = Cookies.get('accessToken');
  //   if (!accessToken) {
  //     setConfirmMessage('Please login first to proceed to checkout');
  //     return;
  //   }
  ...
  const proceedToCheckout = async () => {
    ...
    // Fetch customer data if not yet loaded
    let customerDataToSend = getcustomerdata;
    if (!customerDataToSend) {
      try {
        const data = await getCustomers.getCustomers(customer?.email || '');
        customerDataToSend = data as unknown as Customer;
        setGetCustomerData(customerDataToSend);
      } catch (err) {
    ...
    navigate('/checkout', {
      state: { customerData: customerDataToSend, cartItems: items },
    });
  };
                    ...
                    >
                      ...
                      <div className='cart-item-details'>
                        <div className='cart-item-info-enhanced'>
                          <h4 className='cart-item-name-enhanced'>
                            {item.product.name}
                          </h4>
